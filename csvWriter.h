...
public:
    ...
    CSVWriter(const std::string &separator) {
        this->firstRow = true;
    ...
    CSVWriter(const std::string &separator, int numberOfColumns) {
        this->firstRow = true;
        this->separator = separator;
        this->columnNum = numberOfColumns;
        this->valueCount = 0;
    ...
    CSVWriter &add(std::string str) {
        ...
        if (foundQuotationMarks) {
            str = "\"" + str + "\"";
        } else if (str.find(this->separator) != std::string::npos) {
    ...
    bool writeToFile(const std::string &filename, bool append) {
        ...
        if (append) {
            ...
            fin.open(filename);
            if (fin.is_open()) {
                fin.seekg(-1, std::ios_base::end); //go to t_end of file
